...
```bash
yay -S kitty polybar rofi bspwm-rounded-corners-git xdg-user-dirs thunar xorg pavucontrol blueberry feh lxappearance file-roller gtk-engines gtk-engine-murrine neofetch imagemagick parcellite xclip maim gpick curl jq tint2 zsh moreutils recode dunst plank python-xdg redshift mate-polkit xfce4-settings yaru-sound-theme fish alsa-utils sddm xorg-xinit brightnessctl acpi mugshot playerctl python-pytz glava wmctrl i3lock-color jgmenu inter-font networkmanager-dmenu-git conky-lua bsp-layout zscroll noise-suppression-for-voice starship lsof gamemode lib32-gamemode xdo bluez bluez-utils bluez-libs bluez-tools noto-fonts-emoji ibus-uniemoji
```
